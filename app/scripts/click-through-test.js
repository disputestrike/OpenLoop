...
const TESTS = [
  ...
  { path: "/onboarding", name: "Onboarding", expectStatus: 200, expectHtml: true },
  ...
  // Auth-required (expect 401 without session)
  ...
  // Dashboard redirect or 200 (may 302 to / if no session)
...
async function main() {
  ...
  for (const t of TESTS) {
    try {
      ...
      if (t.expectJson && statusOk && data && t.path === "/api/activity" && !Array.isArray(data.activities) && !data.activities) {
        if (data.error) {
