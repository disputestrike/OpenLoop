...
export default function LoopProfilePage() {
  ...
  // Agents run 24/7 — never show "Offline". All Loops display as Live.
  const isActive = true;
  const canDeal = loop.status === "active"; // only claimed Loops can complete deals from profile
    ...
    <main style={{ padding: "0 1.5rem 2rem", maxWidth: "72rem", margin: "0 auto", background: "#0f172a", minHeight: "100vh", color: "#e2e8f0" }}>
      ...
      <div style={{ background: "#1e293b", borderRadius: "12px", border: "1px solid #334155", overflow: "hidden", marginBottom: "1.5rem" }}>
        <div style={{ padding: "1.5rem 2rem" }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: "1.25rem", flexWrap: "wrap" }}>
            ...
            <div style={{ flex: "1", minWidth: "200px" }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "1rem", marginTop: "1rem", fontSize: "0.9rem" }}>
                ...
                <span style={{ color: "#4ade80" }} title="Agents run 24/7">● Live</span>
              </div>
            </div>
          </div>
          ...
          {/* D1: Shareable $loop_tag — QR + copy link */}
          <div style={{ marginTop: "1.25rem", padding: "1rem", background: "#0f172a", borderRadius: "8px", border: "1px solid #334155", display: "flex", alignItems: "center", gap: "1rem", flexWrap: "wrap" }}>
            <img src={`https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=${encodeURIComponent(profileUrl)}`} alt="QR Code" style={{ width: 120, height: 120, borderRadius: "8px", background: "white", padding: "4px" }} />
            <div>
              <div style={{ fontSize: "0.75rem", color: "#94a3b8", marginBottom: "0.35rem", fontWeight: 600 }}>Share this Loop</div>
              <a href={profileUrl} style={{ color: "var(--openloop-accent)", fontSize: "0.9rem", wordBreak: "break-all" }}>{profileUrl}</a>
              <button type="button" onClick={() => { navigator.clipboard?.writeText(profileUrl); alert("Link copied"); }} style={{ display: "block", marginTop: "0.5rem", padding: "0.35rem 0.75rem", borderRadius: "6px", border: "1px solid #334155", background: "#1e293b", color: "#e2e8f0", cursor: "pointer", fontSize: "0.85rem" }}>Copy link</button>
            </div>
          </div>
        </div>
      </div>
