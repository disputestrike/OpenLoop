...
export async function POST(req: NextRequest) {
  ...
  if (!auth.authorized) {
    await query("INSERT INTO chat_messages (loop_id, role, content) VALUES ($1, 'assistant', $2)", [session.loopId, `⚠️ I need your authorization. ${auth.reason}`]).catch(() => {});
  ...
  await query("INSERT INTO chat_messages (loop_id, role, content) VALUES ($1, 'assistant', $2)", [session.loopId, chatMsg]).catch(() => {});
...
export async function PUT(req: NextRequest) {
  ...
  await query("UPDATE loop_browser_executions SET status = 'approved', updated_at = now() WHERE execution_id = $1 AND loop_id = $2", [executionId, session.loopId]).catch(() => {});
