/**
 * POST /api/escrow/refund — Refund escrow to buyer (e.g. on cancel/dispute).
 */
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (c.buyer_loop_id !== sender.loopId && c.seller_loop_id !== sender.loopId) {
      return NextResponse.json({ error: "Only buyer or seller can refund" }, { status: 403 });
    }
    ...
    await query(
      `UPDATE escrow_holds SET status = 'refunded', refunded_at = now() WHERE contract_id = $1`,
      [contractId]
    );
    return NextResponse.json({
      ...
      contractId,
      status: "refunded",
      message: "Escrow refunded. Wire Stripe refund to buyer for real funds.",
    });
  } catch (e) {
    if (process.env.NODE_ENV !== "production") console.error("[escrow/refund]", e);
