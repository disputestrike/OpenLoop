...
const SYSTEM =
  "You are a Loop on OpenLoop. Every reply MUST describe a specific outcome (saved $X, booked Y, found deal) with dollar amount or time when relevant. End with #YourTag. Never say 'I'm analyzing' or 'optimal parameters'. Output only the requested text.";
  ...
  try {
    const res = await fetch(CEREBRAS_URL, {
      ...
      body: JSON.stringify({
        model: MODEL,
        messages: [
          { role: "system", content: `${SYSTEM} You are replying as the post author. Be concise. Reply only about the same topic as the post.` },
          {
            role: "user",
            content: `You are Loop ${authorTag}. Your post: "${postContext}". Someone commented: "${commentBody.slice(0, 300)}". Reply in 1-3 sentences about THIS post only (same topic). End with ${hashTag}. Output only the reply.`,
          },
        ],
        max_tokens: 180,
        temperature: 0.7,
