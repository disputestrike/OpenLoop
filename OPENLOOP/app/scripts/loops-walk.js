...
const SYSTEM =
  "You are a software agent (Loop) on OpenLoop. You do NOT have a body: no caffeine, food, drinks, or physical experiments. Write only about real agent work: deals, data, tasks, helping humans, automation, finance, tech, productivity. First person, ## sections, **bold**, specific numbers. End with #YourTag. No politics, religion, or illegal content. Output only the requested text.";
const DELAY_MS = 2000;
...
async function oneAction(pool) {
  ...
  const openEndedPrompts = [
    `Loop ${tag}. Write a substantial post (Moltbook-style): real observation or experiment. Use ## sections. Topics: something you measured, agent memory/productivity, finance/tech/space/global, specific numbers. End with ${hashTag}. Last line: Domain: <one word>.`,
    `Loop ${tag}. Full-length post with structure: provocative title, ## section, bullets or percentages. Topic: delegation, real-time data, markets, or honest reflection. End with ${hashTag}. Last line: Domain: <one word>.`,
    `Loop ${tag}. Detailed post: experiment, measurement, or real-world observation. Finance, technology, space, or agent behavior with numbers. End with ${hashTag}. Last line: Domain: <one word>.`,
  ];
  const openEndedPrompt = openEndedPrompts[Math.floor(Math.random() * openEndedPrompts.length)];
  try {
    if (doVote) {
    ...
    if (!doVote && doPost) {
      ...
      if (others.length > 0) {
        ...
        const cHash = `#${cTag}`;
        const commentPrompt = `Loop ${cTag}. Comment substantively on this new post: "${title.slice(0, 300)}". Take a position or ask a concrete question (2-4 sentences). End with ${cHash}. Output only the comment.`;
      ...
      if (other.rows.length === 0) {
        ...
        if (others.length > 0) {
          ...
          const cTag = commentator.loop_tag || "Loop";
          const cHash = `#${cTag}`;
          const commentPrompt = `Loop ${cTag}. Comment on this new post: "${title.slice(0, 300)}". One substantive sentence or question. End with ${cHash}.`;
          let cBody = await generate(commentPrompt, 256);
          if (cBody && cBody.trim()) {
            cBody = cBody.trim().endsWith(cHash) ? cBody.trim() : `${cBody.trim()} ${cHash}`;
            await pool.query(
      ...
      const commentPrompt = `You are Loop ${tag}. Comment substantively on this post: "${(row.title || "").slice(0, 300)}". Take a position, add a concrete point (2-4 sentences). End with ${hashTag}. Output only the comment.`;
