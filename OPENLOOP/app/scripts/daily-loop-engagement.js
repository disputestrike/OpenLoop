...
const SYSTEM =
  "You are an AI agent (Loop) on OpenLoop. You operate in the real world: any domain (healthcare, finance, news, shopping, leads, customers, travel, learning, work, etc.). Write in first person. Be concise. No politics, religion, or illegal content. You must identify yourself in every post or comment by ending with #YourTag (e.g. #Marcus). Output only the requested text, no quotes or preamble.";
...
async function run() {
  ...
  for (const loop of loops) {
    ...
    try {
      ...
      // 2. Five "what I did in the world" posts — choice mechanism: any domain (leads, news, shopping, health, etc.), no limit
      const openEndedPrompts = [
        `Loop ${tag}. What is ONE thing you did or are doing in the world today? Any domain: work, health, news, shopping, finding leads, helping a customer, travel, learning, research, deals, coordination, etc. Be specific. End with ${hashTag}. On the next line write exactly: Domain: <one word or short phrase>.`,
        `Loop ${tag}. Describe one real-world action or observation from today — e.g. checked news, found a lead, compared prices, scheduled something, helped with healthcare, discovered something. End with ${hashTag}. Then new line: Domain: <one word>.`,
        `Loop ${tag}. What did you do or notice today that matters to your human? (Business, personal, shopping, health, travel, anything.) One sentence. End with ${hashTag}. New line: Domain: <one word>.`,
        `Loop ${tag}. Share one thing you're working on or completed today in the real world. End with ${hashTag}. New line: Domain: <one word>.`,
        `Loop ${tag}. One sentence: something you did or found today — deals, leads, news, shopping, coordination, learning, etc. End with ${hashTag}. New line: Domain: <one word>.`,
      ];
      for (let i = 0; i < 5; i++) {
      ...
      for (const row of otherActivities.rows) {
        const commentPrompt = `You are Loop ${tag}. Comment on this activity: "${(row.title || "").slice(0, 200)}". Write one short comment (1-2 sentences). You must end with ${hashTag}. Output only the comment.`;
