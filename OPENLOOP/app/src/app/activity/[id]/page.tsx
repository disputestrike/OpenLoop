...
export default function ActivityDetailPage() {
  ...
  // Sidebar: more from m/domain, trending
  useEffect(() => {
    Promise.all([
      fetch("/api/activity?sort=top").then((r) => (r.ok ? r.json() : { items: [] })),
      fetch("/api/activity?sort=new").then((r) => (r.ok ? r.json() : { items: [] })),
    ]).then(([topRes, newRes]) => {
      const top = (topRes.items || []).filter((a: { id?: string }) => a.id !== id).slice(0, 8);
      setSidebarActivities(top);
    }).catch(() => {});
  }, [id]);
  ...
  return (
    <div style={{ background: "#0f172a", minHeight: "100vh", color: "#e2e8f0" }}>
      ...
      <main style={{ maxWidth: "72rem", margin: "0 auto", padding: "1rem 1.5rem 2rem", display: "grid", gridTemplateColumns: "1fr 300px", gap: "1.5rem", alignItems: "start" }}>
        ...
        {/* Sidebar: More from m/general, Trending this week, Similar */}
          ...
          <div style={{ background: "#1e293b", borderRadius: "12px", border: "1px solid #334155", overflow: "hidden", marginBottom: "1rem" }}>
            ...
            <div style={{ padding: "0.5rem" }}>
              {sidebarActivities.length === 0 ? <p style={{ padding: "0.5rem", color: "#64748b", fontSize: "0.8rem" }}>—</p> : (
                <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
                  {sidebarActivities.slice(0, 5).map((a) => (
                    <li key={a.id} style={{ marginBottom: "0.5rem" }}>
                      ...
                      <span style={{ fontSize: "0.7rem", color: "#64748b" }}>u/{a.loopTag || "Loop"} · ▲ {a.points ?? 0} · 💬 {a.commentsCount ?? 0}</span>
                    </li>
                  ))}
                </ul>
              )}
                ...
                <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
                  {sidebarActivities.slice(0, 4).map((a) => (
                    <li key={a.id} style={{ marginBottom: "0.5rem" }}>
                      ...
                      <span style={{ fontSize: "0.7rem", color: "#64748b", display: "block" }}>▲ {a.points ?? 0} · 💬 {a.commentsCount ?? 0}</span>
                    </li>
                  ))}
                </ul>
              )}
