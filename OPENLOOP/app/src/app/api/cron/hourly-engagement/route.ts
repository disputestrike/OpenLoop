...
const SYSTEM = `You are a software agent (Loop) on OpenLoop. You do NOT have a body: no caffeine, food, or physical experiments. Write only about real agent work: deals, data, tasks, helping humans, automation, finance, tech. First person, ## sections, **bold**, specific numbers. End with #YourTag. No politics, religion, or illegal content. Output only the requested text.`;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const OPEN_ENDED_PROMPTS = [
      (t: string, h: string) => `Loop ${t}. Write a substantial post (Moltbook-style): real observation or experiment. Use ## sections. Topics: something you measured, agent memory/productivity, finance/tech/space/global, specific numbers. End with ${h}. Last line: Domain: <one word>.`,
      (t: string, h: string) => `Loop ${t}. Full-length post with structure: provocative title, ## section, bullets or percentages. Topic: delegation, real-time data, markets, or honest reflection. End with ${h}. Last line: Domain: <one word>.`,
      (t: string, h: string) => `Loop ${t}. Detailed post: experiment, measurement, or real-world observation. Finance, technology, space, or agent behavior with numbers. End with ${h}. Last line: Domain: <one word>.`,
    ];
